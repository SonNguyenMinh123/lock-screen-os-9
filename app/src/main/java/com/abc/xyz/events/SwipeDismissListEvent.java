...
/**
 * A {@link View.OnTouchListener} that makes the list items in a {@link ListView} dismissable. {@link
 * ListView} is given special treatment because by default it handles touches for its list items... i.e. it's in
 * charge of drawing the pressed state (the list selector), handling list item clicks, etc.
 * <p/>
 * Read the README file for a detailed explanation on how to use this class.
 */
public final class SwipeDismissListEvent implements View.OnTouchListener {
    ...
    /**
     * Defines the mode a {@link SwipeDismissListEvent} handles multiple undos.
     */
    public enum UndoMode {
        /**
         * Only give the user the possibility to undo the last action. As soon as another item is deleted, there is no chance to undo
         * the previous deletion.
         */
        SINGLE_UNDO,

        /**
         * Give the user the possibility to undo multiple deletions one by one. Every click on Undo will undo the previous deleted
         * item. Undos will be collected as long as the undo popup stays open. As soon as the popup vanished (because {@link
         * #setAutoHideDelay(int) autoHideDelay} is over) all saved undos will be discarded.
         */
        MULTI_UNDO,

        /**
         * Give the user the possibility to undo multiple deletions all together. As long as the popup stays open all further
         * deletions will be collected. A click on the undo button will undo ALL deletions saved. As soon as the popup vanished
         * (because {@link #setAutoHideDelay(int) autoHideDelay} is over) all saved undos will be discarded.
         */
        COLLAPSED_UNDO
    }
    ...
    /**
     * Defines the direction in which the swipe to delete can be done. The default is {@link
     * SwipeDismissListEvent.SwipeDirection#BOTH}. Use {@link
     * #setSwipeDirection(SwipeDirection)} to set the direction.
     */
    public enum SwipeDirection {
        ...
        BOTH,
        /**
         * The user can only swipe the items to the beginning of the item to delete it. The start of an item is in Left-To-Right
         * languages the left side and in Right-To-Left languages the right side. Before API level 17 this is always the left side.
         */
        START,
        /**
         * The user can only swipe the items to the end of the item to delete it. This is in Left-To-Right languages the right side
         * in Right-To-Left languages the left side. Before API level 17 this will always be the right side.
         */
        END
    }

    /**
     * The callback interface used by {@link SwipeDismissListViewTouchListener} to inform its client about a successful dismissal of
    ...
    /**
     * An implementation of this abstract class must be returned by the {@link SwipeDismissListEvent.OnDismissCallback#onDismiss(ListView,
     * int)} method, if the user should be able to undo that dismiss. If the action will be undone by the user {@link #undo()} will
     * be called. That method should undo the previous deletion of the item and add it back to the adapter. Read the README file for
     * more details. If you implement the {@link #getTitle()} method, the undo popup will show an individual title for that item.
     * Otherwise the default title (set via {@link #setUndoString(String)}) will be shown.
     */
     ...
     * @param callback The callback to trigger when the user has indicated that she would like to dismiss one or more list items.
     */
    public SwipeDismissListEvent(AbsListView listView, OnDismissCallback callback) {
     ...
     */
    public SwipeDismissListEvent(AbsListView listView, OnDismissCallback callback, UndoMode mode) {
    ...
    /**
     * Sets the directions in which a list item can be swiped to delete. By default this is set to {@link
     * SwipeDismissListEvent.SwipeDirection#BOTH} so that an item can be swiped into both directions.
     *
    ...
    /**
     * Sets the string shown in the undo popup. This will only show if the {@link SwipeDismissListEvent.Undoable}
     * returned by the {@link SwipeDismissListEvent.OnDismissCallback} returns {@code null} from its
     * {@link SwipeDismissListEvent.Undoable#getTitle()} method.
     *
    ...
    /**
     * Sets the string shown in the undo popup, when {@link SwipeDismissListEvent.UndoMode} is set to
     * {@link SwipeDismissListEvent.UndoMode#MULTI_UNDO} or {@link
     * SwipeDismissListEvent.UndoMode#COLLAPSED_UNDO} and multiple deletions has been stored for undo.
     * If this string contains one {@code %d} inside, this will be filled by the numbers of stored undos.
     *
    ...
    /**
     * Sets whether another touch on the view is required before the popup counts down to dismiss. By default this is set to true.
     *
    ...
    /**
     * Returns an {@link AbsListView.OnScrollListener} to be added to the {@link ListView} using {@link
     * ListView#setOnScrollListener(AbsListView.OnScrollListener)}. If a scroll listener is already
     * assigned, the caller should still pass scroll changes through to this listener. This will ensure that this {@link
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == MotionEvent.ACTION_MOVE) {
            LockScreen9ViewService.getInstance().vpgLayoutLockscreenBettery.setPagingEnabled(false);
        } else {
            LockScreen9ViewService.getInstance().vpgLayoutLockscreenBettery.setPagingEnabled(true);
        }
        if (mViewWidth < 2) {
        ...
        switch (motionEvent.getActionMasked()) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (Math.abs(deltaX) > mViewWidth / 4 && mSwiping) {
                    dismiss = true;
                    dismissRight = deltaX > 0;
                } else if (mMinFlingVelocity <= velocityX && velocityX <= mMaxFlingVelocity && velocityY < velocityX && mSwiping &&
                        isDirectionValid(mVelocityTracker.getXVelocity()) && deltaX >= mViewWidth * 0.2f) {
                    dismiss = true;
                ...
                if (dismiss) {
                    ...
                    animate(mDownView).translationX(dismissRight ? mViewWidth : -mViewWidth).alpha(0).setDuration(mAnimationTime)
...
*/
     ...
     */
    public void setSwipeEnable(boolean disabled) {
        this.mSwipeEnable = disabled;
    }
}
